...
public class BSPageBar
{
	...
	// 페이지 네비게이션 초기화
	/*
	 * 화면에서 받아와야 하는 정보에는 어떤 것들이 있을까? 페이지에 뿌려질 로우의 수 numberPerPage 전체 레코드 수 totalRecord 현재 내가 바라보는 페이지 번호 nowPage 내가 처리해야할 페이지 이름 pagePath
	 * 
	 * 공식을 세우는데 필요한 인자는 누구?
	 * 
	 * 세워진 공식들은 어디에서 적용하면 되는 거지?
	 * 
	 * 화면에 내보내 져야 하는 언어는 html 아님 자바 중에서 ????? html 내보내지는 정보는 어디에 담으면 될까? 생성자 선언(파라미터가 4개가 필요해. 왜냐면 생성자는 메소드 오버로딩의 규칙을 따르니까
	 */
	/*******************************************************************************************************
	 * 파라미터에 있는 변수들은 모두 전변으로 선언되어 있다. 생성자를 갖는다는 건 전변을 초기화 해주는 역할
	 * 
